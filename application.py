...
from DBManager.NoteInfo import upload_note, get_note, update_note, delete_note
...
class Application(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 富文本
            ...
            if status == 200:
                ...
                if bool(int(data['read_permission'])):
                    url = f"http://{self.headers['Host']}{self.path}"
                    product_uuid = get_url_data(url)['product_uuid'][0]
                    response_code, message = get_product_detail(product_uuid)
                    bmessage = message.encode("utf-8")
                    self.send_response(response_code)
                    self.send_header('Content-type', 'text/html')
                    self.end_headers()
    ...
    def do_POST(self):
        ...
        # 上传网站信息
            ...
            if status == 200:
                ...
                if bool(int(data['upload_permission'])):
                    ...
                    post_data = self.rfile.read(content_length).decode("utf-8")
                    data = json.loads(post_data)
                    response_code, message = upload_product(data)
                    bmessage = message.encode("utf-8")
                    # try:
                    #
                    # except Exception:
        ...
        elif self.path == "/api/product_class":
            username, status, message = self.auth()
            if status == 200:
                data = permission_status(username)
                if bool(int(data['upload_permission'])):
        ...
        # 广告修改
            ...
            if status == 200:
                ...
                if bool(int(data['update_permission'])):
                    ...
                    post_data = self.rfile.read(content_length).decode("utf-8")
                    data = json.loads(post_data)
                    response_code, message = update_product_class(data)
                    bmessage = message.encode("utf-8")
                    # try:
                    #
                    # except Exception:
            ...
            if status == 200:
                ...
                if bool(int(data['update_permission'])):
                    ...
                    post_data = self.rfile.read(content_length).decode("utf-8")
                    data = json.loads(post_data)
                    response_code, message = update_product(data)
                    bmessage = message.encode("utf-8")
                    # try:
                    #
                    # except Exception:
        ...
        if self.path == "/api/rich_text":
            ...
            if status == 200:
                ...
                if bool(int(data['update_permission'])):
                    ...
                    post_data = self.rfile.read(content_length).decode("utf-8")
                    data = json.loads(post_data)
                    response_code, message = delete_ad_text(data)
                    bmessage = message.encode("utf-8")
                    # try:
                    #
                    # except Exception:
